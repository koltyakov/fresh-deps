...
export function condaVersions(doc: CondaPackage, subdir: string): RegistryVersions {
  ...
  const published = [...new Set(doc.files.flatMap((file) => file.version && condaScheme.isVersion(file.version)
    && (!file.labels || file.labels.includes('main')) ? [file.version] : []))];
  return { all, published, allComplete: true, latest: condaScheme.max(all, { includePrerelease: false }) };
}
  ...
  async fetchVersions(name: string, channels: string): Promise<RegistryVersions> {
    for (const channel of channels.split('|')) {
      if (channel === 'defaults' || channel.startsWith('https://') || channel.includes('/label/')) {
        ...
        for (const base of bases) {
          ...
          for (const subdir of [this.subdir, 'noarch']) {
            ...
            if (!doc || !doc.packages && !doc['packages.conda']) allComplete = false;
          ...
          if (all.length) return { all: [...new Set(all)], allComplete, source: `${base} [${this.subdir}, noarch]`, latest: condaScheme.max(all, { includePrerelease: false }) };
        }
        continue;
      }
      ...
      if (result.all?.length || doc.files.some((file) => [this.subdir, 'noarch'].includes(file.attrs?.subdir ?? file.basename?.split('/')[0] ?? '')
        && (!file.labels || file.labels.includes('main')))) return { ...result, source: `https://conda.anaconda.org/${channel}` };
    }
