  ...
  async fetchVersions(name: string, sources?: string): Promise<RegistryVersions> {
    if (sources) {
      for (const source of sources.split('|')) {
        ...
        if (!index) continue;
        if (index.packages?.[name]) return { ...packagistVersions({ packages: { [name]: Object.values(index.packages[name]) } }, name), source };
        ...
        if (doc?.packages?.[name]?.length) return { ...packagistVersions(doc, name), source };
      }
...
export function packagistVersions(doc: PackagistResponse, name: string): RegistryVersions {
  if (!Array.isArray(doc.packages?.[name])) return { error: 'no comparable versions found' };
  ...
  return all.length ? { all, latest, allComplete: Array.isArray(doc.packages?.[name]), meta: release ? {
  ...
  } : undefined } : { all, allComplete: Array.isArray(doc.packages?.[name]) };
}
