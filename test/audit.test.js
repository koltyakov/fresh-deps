...
const { AuditCache } = require('../out/audit');
...
const clean = { status: 'checked', advisories: [] };
const versionKey = 'npm|https://registry.example|pkg';

function setup(t, spec = '1.0.0') {
  ...
  const audit = t.mock.method(NpmClient.prototype, 'fetchAudit', async () => vulnerable);
...
for (const [label, response] of [['clean', clean], ['vulnerable', vulnerable], ['unsupported', { status: 'unsupported' }]]) {
...
for (const ttl of [30_000, 120_000]) {
  test(`audit failures expire after the shorter of ${ttl}ms and one minute`, async (t) => {
    ...
    t.mock.method(Date, 'now', () => now);
    const cache = new AuditCache();
    const failure = { status: 'failed', error: 'offline' };
